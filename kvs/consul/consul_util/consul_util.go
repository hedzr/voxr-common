...
func getClientImpl(config *store.ConsulConfig) *api.Client {
...
func GetConsulApiEntryPoint(registrar *Registrar) *api.CatalogService {
	var err error = nil
	registrar.Clients, err = QueryService(store.SERVICE_CONSUL_API, registrar.FirstClient.Catalog())
	if err != nil {
		log.Fatal(fmt.Errorf("err: %v", err))
		return nil
	} else {
...
func findConsulApi(base *Base) []*api.CatalogService {
	services, err := QueryService(store.SERVICE_CONSUL_API, base.FirstClient.Catalog())
	if err != nil {
		log.Errorf("err: %v", err)
		return nil
	} else {
