...
func (s *KVStoreConsul) PutYamlLite(key string, value interface{}) error {
	...
	err = s.PutLite(key, tool.UnescapeUnicode(b))
	if err != nil {
		warnf("ERR: %v", err)
		return err
	}
...
func mapCast(m map[string]interface{}) (ret map[string]string) {
...
func (s *KVStoreConsul) Register(serviceName, id, version string, ipOrHost net.IP, port int, ttl int64, tags []string, meta map[string]interface{}, moreChecks api.AgentServiceChecks) error {
	...
	agentService, err := consul_util.QueryServiceByID(id, s.Client)
	...
	if len(s.ServiceID) == 0 {
		s.ServiceID = id
		cmdr.Set("server.serviceId", id)
	}
	log.Infof("REGISTER OK. '%s' at '%s'", id, addr)

	if ttl > 0 {
		// TTL 心跳 / 撤销注册
		go s.heartbeatRoutine(serviceName, id, version, addr, ipOrHost, port, ttl, tags, meta, moreChecks)
	}
	return nil
}
...
// 的go routine无法被全部释放
func (s *KVStoreConsul) heartbeatRoutine(serviceName, serviceID, version string, addr string, ipOrHost net.IP, port int, ttl int64, tags []string, meta map[string]interface{}, moreChecks api.AgentServiceChecks) {
	...
	for !shouldBeBreak {
		...
		case <-heartbeat:
			...
			if err := s.Client.Agent().UpdateTTL(checkId, "OK", api.HealthPassing); err != nil {
				log.Warnf("      [heartbeat] TTL update failed. WARN-ERR: %v", err)
				// 尝试再次注册自己，如果是因为服务注册信息以外遭到破坏的话
				if err := s.Register(serviceName, serviceID, version, ipOrHost, port, ttl, tags, meta, moreChecks); err != nil {
					log.Errorf("      [heartbeat] TTL update failed. ERR: %v", err)
				} else {
					shouldBeBreak = true
				}
			}
...
func deregisterServiceFast(cc *api.Client, serviceName string, all bool, ipOrHost net.IP, port int) (err error) {
	var err1 error
	consul_util.WaitForResult(func() (bool, error) {
...
func deregisterService(cc *api.Client, serviceName string, all bool, ipOrHost net.IP, port int) (err error) {
	...
	for _, s := range services {
		...
		if !consul_util.Contains(sPassing, s) {
			del = true
		}
		if !all {
...
func (s *KVStoreConsul) NameResolver(serviceName, ver, what string) (net.IP, uint16, string) {
	...
	services, err := consul_util.QueryService(serviceName, s.Client.Catalog())
...
func (s *KVStoreConsul) NameResolverAll(serviceName string, what string) (sr []*store.ServiceRecord) {
	...
	// TODO 尚未处理 addr，grpc 等情况，尚未处理如何同时注册，以及提取问题。
	if len(what) == 0 {
		what = "addr"
	}
